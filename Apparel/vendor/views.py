...
def signup(request):
	...
	if request.method == "POST":
		...
		if (password == confirm_password):
			...
			reg.save()
			subject = "Apparel Vendor Verification"
			message = "Hello " + name + ", " + '\n' + "Welcome to Apparel. Please verify your email id.. http://127.0.0.1:8000/vendor/confirm_email"
			email_from = settings.EMAIL_HOST_USER
			email_to = [email,]
			send_mail(subject,message,email_from,email_to)
			return redirect('Vendor:verification_sent')
		else:
			message.error(request,"{}, Please correct error below..".format(name))
	template = "vendor/account/signup.html"
...
def change_password(request):
    ...
    if request.session.get('vendor') == None:
        messages.error(request,'You Must have to login first')
        return redirect('Vendor:login')
